...
static function bool IsLocalPlayer(ETeam TeamFlag)
{
    switch (TeamFlag)
    {
        ...
        case eTeam_One:
            return `DC_CFG(bPlayerControlsTeamOneTurn);
        case eTeam_Two:
            return `DC_CFG(bPlayerControlsTeamTwoTurn);
        default:
            return false;
    }
}
...
static function bool IsPlayerControllingUnit(XComGameState_Unit UnitState)
{
    ...
    if (UnitState.IsUnrevealedAI() && !UnitState.IsMindControlled())
    {
        switch (UnitState.GetTeam())
        {
            ...
            case eTeam_One:
                return `DC_CFG(bPlayerControlsUnactivatedTeamOne);
            case eTeam_Two:
                return `DC_CFG(bPlayerControlsUnactivatedTeamTwo);
            default:
                return false;
        }
    }
