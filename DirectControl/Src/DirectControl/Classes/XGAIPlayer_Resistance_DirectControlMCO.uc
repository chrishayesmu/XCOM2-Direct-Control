...
function Init(bool bLoading = false)
{
    ...
    // In case we're loading a save made during a Resistance turn, we need to set the controlling team
    if (class'DirectControlUtils'.static.GetActivePlayer().TeamFlag == eTeam_Resistance && class'DirectControlUtils'.static.IsLocalPlayer(eTeam_Resistance))
    {
        class'DirectControlUtils'.static.SetControllingPlayerTeam(eTeam_Resistance);
    }
}
...
simulated function OnBeginTurnVisualized(int InitiativeObjectID)
{
    ...
    {
        if (class'DirectControlUtils'.static.IsLocalPlayer(eTeam_Resistance))
        {
            class'DirectControlUtils'.static.SetControllingPlayerTeam(eTeam_Resistance);
        }
    }
...
simulated function OnUnitActionPhase_ActionsAvailable(XComGameState_Unit UnitState)
{
	...
	{
        // The Resistance turn overlay normally won't hide, but it's blocking the ability bar
    ...
    if (!`PRES.m_kTurnOverlay.IsShowingXComTurn())
    {
        `PRES.m_kTurnOverlay.ShowXComTurn();
    }
